...
public class SyncClient : MonoBehaviour {
	...
	void UpdateInformation(){
		if (!string.IsNullOrEmpty (playerInfo.Get ("name"))) {
			...
			ScoreUI.transform.Find ("NameInputField").GetComponent<InputField> ().text = name;
		} else
			ScoreUI.transform.Find ("NameInputField").GetComponent<InputField> ().text = "";
		if (!string.IsNullOrEmpty (playerInfo.Get ("health"))) {
			health = int.Parse(playerInfo.Get ("health"));
			ScoreUI.transform.Find ("HealthInputField").GetComponent<InputField> ().text = health.ToString();
		} else
			ScoreUI.transform.Find ("HealthInputField").GetComponent<InputField> ().text = "";
		if (!string.IsNullOrEmpty (playerInfo.Get ("exp"))) {
			exp = int.Parse (playerInfo.Get ("exp"));
			ScoreUI.transform.Find ("ExpInputField").GetComponent<InputField> ().text = exp.ToString ();
		} else
			ScoreUI.transform.Find ("ExpInputField").GetComponent<InputField> ().text = "";
	}
